...
test('should render search component', () => {
    render(<Provider store={store}><MemoryRouter><Search /></MemoryRouter></Provider>)
});

test('should shows the error', async () => {
...
test('should shows the Books when search', async () => {
    const {findByText} = render(
         <Provider store={store}>
            <MemoryRouter>
             <BookItem authors={searchbooks[0].authors} id={searchbooks[0].id} title={searchbooks[0].title} imageLinks={searchbooks[0].imageLinks} shelf={searchbooks[0].shelf} />
             </MemoryRouter>
         </Provider>
     );
    expect(await findByText('The Linux Command Line')).toBeInTheDocument();
});

test('should test onClick behavior to clear search', async () => {
    await act( async () => render(<Provider store={store}><MemoryRouter><Search /></MemoryRouter></Provider>));
    const backButton = screen.getByRole('back');
    backButton.click();

    expect(await store.getState().books.searchBooks).toHaveLength(0);
});
